...
import { LogoCompany } from "@/public/svg/logo"
import { IconClose, IconHamburger } from "@/public/svg/icon"
...
export default function Header({ theme = "dark" }: HeaderProps) {
  ...
  const hoverTextColor = theme === "dark" ? "hover:text-gray-300" : "hover:text-gray-600"
  const bgColor = theme === "dark" ? "bg-[#FFFFFF1A]" : "bg-[#00000010]"
  const iconColor = theme === "dark" ? "text-white" : "text-gray-900"
  const iconHoverBg = theme === "dark" ? "hover:bg-white" : "hover:bg-gray-900"
  const iconHoverColor = theme === "dark" ? "group-hover:text-black" : "group-hover:text-white"
  const mobileBg = theme === "dark" ? "bg-black bg-opacity-90" : "bg-white bg-opacity-95"
  const phoneIconBg = theme === "dark" ? "bg-white" : "bg-gray-900"
  const phoneIconColor = theme === "dark" ? "text-black" : "text-white"
    ...
    <header className="pt-6 pb-6 relative z-50">
      <div className="max-w-[1632px] mx-auto px-5 flex items-center justify-between">
        <div className="flex items-center justify-center gap-8">
          <Link href="/#">
            <LogoCompany color={theme == "dark" ? "white" : "black"} />
          </Link>
          <ul className={`${textColor} xl:flex hidden gap-8`}>
            <li>
              <Link
                href="/#service"
                className={`text-base font-normal leading-[100%] ${hoverTextColor} uppercase transition-colors`}
              >
                Xizmatlarimiz
              </Link>
            </li>
        ...
        <div className="xl:flex hidden flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className={`${bgColor} p-1 flex items-center rounded-full backdrop-blur-[100px] w-full sm:max-w-44`}>
            {/* Facebook */}
            <Link href="/#" className={`group w-14 h-14 p-4 rounded-full ${iconHoverBg} transition`}>
              <svg
                className={`w-6 h-6 ${iconColor} ${iconHoverColor} transition`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
          ...
          <Link
            ...
            className={`flex items-center gap-2 ${bgColor} pl-1 py-1 rounded-full backdrop-blur-[100px] w-full sm:w-auto`}
          >
            <span className={`group p-4 rounded-full hover:bg-transparent ${phoneIconBg} transition`}>
              <svg
                className={`w-6 h-6 group-hover:${iconColor} ${phoneIconColor} transition`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <span className={`text-xl leading-[100%] font-medium ${textColor} py-5 pr-4`}>+998 90 000 00 00</span>
          </Link>
        </div>

        <div className="xl:hidden block">
          <button onClick={toggleMobileMenu}>
            <IconHamburger color={theme == "dark" ? "white" : "black"} />
          </button>
        </div>
      </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`xl:hidden fixed inset-0 ${mobileBg} ${textColor} px-5 py-6 transform transition-opacity duration-300 ease-in-out z-40 ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"
          }`}
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={closeMobileMenu} className="text-3xl cursor-pointer">
            &times;
          </button>
        </div>
        <ul className="flex flex-col gap-5">
          <li>
            <Link
              href="/#service"
              className={`text-base font-normal uppercase ${hoverTextColor} transition-colors`}
        ...
        <div className="mt-35 space-y-4">
          <div className={`${bgColor} p-1 flex items-center rounded-full backdrop-blur-[100px] w-full max-w-44`}>
            <Link href="/#" className={`group p-4 rounded-full ${iconHoverBg} transition`}>
              <svg
                className={`w-6 h-6 ${iconColor} ${iconHoverColor} transition`}
                fill="currentColor"
                viewBox="0 0 24 24"
          ...
          <Link
            href="tel:+998900000000"
            className={`flex items-center gap-2 ${bgColor} pl-1 py-1 rounded-full backdrop-blur-[100px] max-w-63.75`}
          >
            <span className={`group p-4 rounded-full hover:bg-transparent ${phoneIconBg} transition`}>
              <svg
                className={`w-6 h-6 group-hover:${iconColor} ${phoneIconColor} transition`}
